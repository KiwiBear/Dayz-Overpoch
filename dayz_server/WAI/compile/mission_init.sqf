if(isServer) then {
	...
	private ["_mines","_difficulty","_mission","_type","_color","_dot","_position","_marker","_border","_name","_show_marker"];
  ...
  _border ="";
	...
	call {
		if(_difficulty == "easy")		exitWith {_color = "ColorKhaki"};
		if(_difficulty == "medium")		exitWith {_color = "ColorOrange"};
	...
	if(_show_marker) then {
		...
		[_position, _color, _name, _mission] spawn {
			...
			private["_position","_color","_name","_running","_mission","_type","_marker","_border","_dot"];
            ...
            //added by blueTesla   
        _border 		= createMarker [_type + str(_mission) + "brd", _position];
				_border 		setMarkerColor "ColorBlack";
				_border 		setMarkerShape "ELLIPSE";
				_border 		setMarkerBrush "Border";
				_border 		setMarkerSize [300,300];
