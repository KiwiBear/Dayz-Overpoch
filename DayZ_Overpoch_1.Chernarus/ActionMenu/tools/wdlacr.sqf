//*******************Upgrade ACR >> ACR TWS (Woodland) : 1x ACR, 1x Lasermarker, 1x Laserbattery, 1x Obsidian and 15k humanity************************


_weaponsPlayer = weapons player; 
_itemsPlayer = items player;
_magazinesPlayer = magazines player;
_hasACR1w = "FHQ_ACR_WDL_IRN" in _weaponsPlayer;            //Weapon
_hasACR2w = "FHQ_ACR_WDL_IRN_F" in _weaponsPlayer;         //Alternative Weapon Version
...
if !((_hasACR1w && _hasLaserM && _hasLaserB && _hasObsidian) or ( _hasACR2w && _hasLaserM && _hasLaserB && _hasObsidian )) exitWith {cutText [format["You need 1x ACR (Woodland), 1x Lasermarker, 1x Laserbattery, 1x Obsidian to upgrade from ACR (Woodland) to ACR TWS (Woodland) ! The Upgrade will cost you 15000 humanity!"], "PLAIN DOWN"];};       //works
if (dayz_combat == 1) then { 
  ...
  player removeWeapon "FHQ_ACR_WDL_IRN";
  player removeWeapon "FHQ_ACR_WDL_IRN_F";
	player removeWeapon "Laserdesignator";
	player removeMagazine "Laserbatteries";
  player removeMagazine "ItemObsidian";
  sleep 1;
  player addWeapon "FHQ_ACR_WDL_TWS_F";
  cutText [format["You successfully upgraded your ACR (Woodland) to a ACR TWS (Woodland)"], "PLAIN DOWN"];
  sleep 3;
